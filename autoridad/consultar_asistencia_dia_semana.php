...
if ($num_registros > 0) {
    ...
    if ($num_registros > 0) {
        ...
        while ($row = $db->fetch_object($consulta)) {
            ...
            // Obtener asistencia registrada por el tutor
            $query = "SELECT at.*, in_nombre 
                        FROM sw_asistencia_tutor at, 
                             sw_inasistencia i
                       WHERE i.id_inasistencia = at.id_inasistencia 
                         AND id_estudiante = $id_estudiante 
                         AND id_paralelo = $id_paralelo 
            ...
            if ($num_registros > 0) {
                ...
                // No existe el registro de asistencia por parte del tutor
                $justificacion = "";
                $observacion = "NO HAY REGISTRO DE PARTE DEL TUTOR";
                $cadena .= "<td style='color: red;'><i class='fa fa-fw fa-remove'></i></td>\n";
            }

            // Desplegar la observación
            $cadena .= "<td>$observacion</td>\n";
            $cadena .= "<td>$justificacion</td>\n";
            $cadena .= "</tr>\n";
        }
    } else {
        $cadena .= "</thead>\n";
