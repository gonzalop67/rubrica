...
$objPHPExcel->getActiveSheet()->setCellValue('D1', $nombreInstitucion)
                              ...
                              ->setCellValue('D21', $nombreRector)
                              ->setCellValue('O21', $nombreSecretario);

// Renombrar la hoja de calculo
$objPHPExcel->getActiveSheet()->setTitle('CUADRO DE GRACIA');

// Vectores de configuracion para las columnas
$colAsignaturas = array('D', 'G', 'J', 'M', 'P', 'S', 'V', 'Y', 'AB', 'AE', 'AH', 'AK', 'AN', 'AQ', 'AT', 'AW');
$colDeGracia = array('E', 'H', 'K', 'N', 'Q', 'T', 'W', 'Z', 'AC', 'AF', 'AI', 'AL', 'AO', 'AR', 'AU', 'AX');
...
if ($num_total_estudiantes > 0) {
    ...
    while ($estudiante = $db->fetch_assoc($estudiantes)) {
        ...
        $asignaturas = $db->consulta("SELECT as_abreviatura, a.id_asignatura, as_nombre FROM sw_asignatura a, sw_asignatura_curso ac, sw_paralelo p WHERE a.id_asignatura = ac.id_asignatura AND p.id_curso = ac.id_curso AND a.id_tipo_asignatura = 1 AND p.id_paralelo = $id_paralelo ORDER BY ac_orden");
        ...
        if ($total_asignaturas > 0) {
            ...
            while ($asignatura = $db->fetch_assoc($asignaturas)) {
                ...
                $query = $db->consulta("SELECT contar_remediales_no_aprobados($id_periodo_lectivo, $id_estudiante, $id_paralelo) AS total");
                ...
                if($total_remediales == 1){
                    ...
                    if($promedio_anual_truncado >= 7) {
                        $objPHPExcel->getActiveSheet()->setCellValue($colAsignaturas[$contAsignatura].$row, $promedio_anual_truncado);
                        $objPHPExcel->getActiveSheet()->setCellValue($colPromedioFinal[$contAsignatura].$row, $promedio_anual_truncado);
                    } else if($promedio_anual_truncado >= 5 && $promedio_anual_truncado < 7) {
                            ...
                            if($calificacion >= 7) {
                                ...
                                $objPHPExcel->getActiveSheet()->setCellValue($colPromedioFinal[$contAsignatura].$row, 7);
                                    ...
                                    if($calificacion >= 7) {
                                        ...
                                        $objPHPExcel->getActiveSheet()->setCellValue($colAsignaturas[$contAsignatura].$row, $promedio_anual_truncado);
                                        if($calificacion == 0) {
                                            $objPHPExcel->getActiveSheet()->setCellValue($colDeGracia[$contAsignatura].$row, "SE");
                                            $objPHPExcel->getActiveSheet()->setCellValue($colPromedioFinal[$contAsignatura].$row, $promedio_anual_truncado);
                                        } else {
                                            $objPHPExcel->getActiveSheet()->setCellValue($colDeGracia[$contAsignatura].$row, $calificacion);
                                            $objPHPExcel->getActiveSheet()->setCellValue($colPromedioFinal[$contAsignatura].$row, 7);
                                        }
                    ...
                    } else if($promedio_anual_truncado > 0 && $promedio_anual_truncado < 5) {
                            // Obtencion de la calificacion del examen remedial
