...
while($asignatura = $db->fetch_assoc($asignaturas))
{
	...
	if($id_tipo_asignatura==1){
		...
		$query = $db->consulta("SELECT calcular_prom_final_cualitativa($id_periodo_lectivo,$id_estudiante,$id_paralelo,$id_asignatura) AS promedio_final");
		$registro = $db->fetch_assoc($query);
		$promedio_final = ceil($registro["promedio_final"]);
		$query = $db->consulta("SELECT ec_abreviatura, ec_equivalencia FROM sw_escala_proyectos WHERE ec_correlativa = $promedio_final");
		$registro = $db->fetch_assoc($query);
		$abreviatura = $registro["ec_abreviatura"];
		$equivalencia = $registro["ec_equivalencia"];
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$row, $abreviatura);
...
if($num_total_registros > 0)
{
	...
	$promedio_anual = truncateFloat($promedio_anual, 2);
}
...
$query = $db->consulta("SELECT id_curso_superior FROM sw_curso WHERE id_curso = $id_curso");
$registro = $db->fetch_assoc($query);
$id_curso_superior = $registro["id_curso_superior"];

$consulta = $db->consulta("SELECT cs_nombre FROM sw_curso_superior WHERE id_curso_superior = $id_curso_superior");
