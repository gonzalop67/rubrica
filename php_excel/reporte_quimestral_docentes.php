...
$docente = $db->consulta("
SELECT CONCAT(us_titulo, ' ' ,
	   SUBSTRING_INDEX(us_nombres, ' ', 1), 
	   SUBSTRING_INDEX(us_apellidos, ' ', 1)) AS us_shortname 
...
");
$registro = $db->fetch_object($docente);
$profesor = $registro->us_shortname;

$tutorCurso = $db->fetch_object($db->consulta("
SELECT CONCAT(us_titulo, ' ' ,
       SUBSTRING_INDEX(us_nombres, ' ', 1), 
	   SUBSTRING_INDEX(us_apellidos, ' ', 1)) AS us_shortname 
  FROM sw_usuario u, 
...
$objPHPExcel->getActiveSheet()->setCellValue('B72', $profesor)
							  ->setCellValue('D72', $tutorCurso);
...
if($num_total_estudiantes > 0)
{
	...
	while($estudiante = $db->fetch_assoc($estudiantes))
	{
		...
		// Calculo el promedio de parciales, su ponderado, el ponderado del examen y la nota quimestral mediante formulas de excel

		$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, "=TRUNC(SUM(C".$row.",D".$row.",E".$row.")/3,2)") //Promedio de los parciales
							    	  ->setCellValue('G'.$row, "=TRUNC(F".$row."*(80/100),2)") //Ponderado de los parciales
							    	  ->setCellValue('I'.$row, "=TRUNC(H".$row."*(20/100),2)") //Ponderado del examen
							    	  ->setCellValue('J'.$row, "=SUM(G".$row.",I".$row.")"); //Nota Quimestral

		$row++;
	}

	// Elimino las filas excedentes
	if($num_total_estudiantes < 60)
		$objPHPExcel->getActiveSheet()->removeRow($row, $filaBase + 60 - $row);
}

$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
$objWriter->save("CUADRO QUIMESTRAL " . str_replace('"','',$nombreParalelo) . " " . $nombrePeriodoLectivo . " " . $nombreAsignatura . ".xls");
...
// Codigo para abrir la caja de dialogo Abrir o Guardar Archivo
	...
	header ("Content-Disposition: attachment; filename=\"" . "CUADRO QUIMESTRAL " . str_replace('"','',$nombreParalelo) . " " . $nombrePeriodoLectivo . " " .$nombreAsignatura . ".xls" . "\"" );
	readfile("CUADRO QUIMESTRAL " . str_replace('"','',$nombreParalelo) . " " . $nombrePeriodoLectivo . " " .$nombreAsignatura . ".xls");
