...
<script type="text/javascript">
	...
	function mostrarTitulosPeriodos()
	{
		$.post("calificaciones/mostrar_titulos_periodos.php", 
			{
				pe_principal: 4, // 4 indica que son examenes de gracia
				alineacion: "center"
			},
			function(resultado)
			{
	...
	function seleccionarParalelo(id_curso, id_paralelo, id_asignatura, asignatura, curso, paralelo)
	{
            ...
            $("#mensaje").html("");
            $("#tituloNomina").html("RESUMEN DE GRACIA [" + asignatura + " - " + curso + " " + paralelo + "]");
            //Aqui va la llamada a la paginacion de estudiantes
            cargarEstudiantesDeGracia(id_paralelo, id_asignatura);
            $("#ver_reporte").css("display","block");
	}

	function cargarEstudiantesDeGracia(id_paralelo, id_asignatura)
	{
		$("#lista_remediales").html("<img src='imagenes/ajax-loader.gif' alt='procesando...' />");
		$("#ver_reporte").css("display","none");
		$.post("scripts/reporte_de_gracia.php", 
			{ 
				id_paralelo: id_paralelo,
			...
			for (var iCont=0; iCont < frmFormulario.length; iCont++) {
				...
				if(objElemento.type=='text') {
					...
					if(fila_elem==fila) {
						...
						var suma_total = document.getElementById("promedio_periodos_"+fila_elem).value;
						suma_total = parseFloat(suma_total) +  parseFloat(calificacion);
						document.getElementById("suma_total_"+fila_elem).value = Math.round(suma_total * 100) / 100;
						// Para el promedio debemos tener en cuenta el Articulo 212 del Reglamento de la LOEI
						if (calificacion < 7) {
							promedio_final = suma_total / 2;
...
<div id="pagina">
    ...
    <div id="pag_nomina_estudiantes" style="margin-top:2px;">
      ...
      <div id="tituloNomina" class="header2"> RESUMEN DE GRACIA </div>
      <div class="cabeceraTabla">
        <table class="fuente8" width="100%" cellspacing=0 cellpadding=0 border=0>
            <tr class="cabeceraTabla">
                <td width="5%">Nro.</td>
