        ...
        <div class="box box-info">
            <div class="box-header with-border">
                <div style="margin-top: 10px;">
                    <form id="form_import" action="../importar_excel.php" method="POST" class="form-horizontal" enctype="multipart/form-data">
                        <div class="form-group">
                            ...
                            <div class="col-sm-2">
                                <button type="submit" id="import_excel" name="import_excel" class="btn btn-info btn-sm"><i class="fa fa-upload"></i> Importar desde Excel</button>
                            </div>
                        </div>
                    </form>
...
<script>
    $(document).ready(function() {
        ...
        $("#form_import").submit(function(e) {
            ...
            const excel_file = document.forms['form_import']['student_file'];
            const validExt = ["xlsx", "XLSX"];

            if (excel_file.value != '') {
                const file_ext = excel_file.value.substring(excel_file.value.lastIndexOf('.') + 1);
                ...
                if (result == false) {
                    Swal.fire({
                        icon: "error",
                        title: "¡Error!",
                        text: "Debe cargar un archivo de tipo Excel.",
                    });
                    cont_errores++;
                    return false;
                } else {
                    const CurrentFileSize = parseFloat(excel_file.files[0].size / (1024 * 1024));
                    if (CurrentFileSize >= 1) {
                        Swal.fire({
                            icon: "error",
                            title: "¡Error!",
                            text: "El archivo Excel debe tener un tamaño máximo de 1 Mb. Tamaño actual: " + CurrentFileSize.toPrecision(4) + " Mb.",
                        });
                        cont_errores++;
                        return false;
                    }
