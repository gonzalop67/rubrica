...
class estudiantes extends MySQL
{
	...
	function obtenerEstadoPeriodoLectivo()
	{
		$qry = "SELECT pe_estado FROM sw_periodo_lectivo WHERE id_periodo_lectivo = " . $this->id_periodo_lectivo;
		$consulta = parent::consulta($qry);
		$pe_estado = parent::fetch_object($consulta)->pe_estado;
		return $pe_estado;
	...
	{
		$consulta = parent::consulta("SELECT * FROM sw_estudiante WHERE es_apellidos = '$apellidos' AND es_nombres = '$nombres'");
		return (parent::num_rows($consulta) > 0);
	}

	function estudianteYaMatriculado($apellidos, $nombres, $id_periodo_lectivo)
	{
		// Primero obtengo el id_estudiante si existe
		$consulta = parent::consulta("SELECT * FROM sw_estudiante WHERE es_apellidos = '$apellidos' AND es_nombres = '$nombres'");
		if (parent::num_rows($consulta) > 0) {
			$registro = parent::fetch_object($consulta);
			$id_estudiante = $registro->id_estudiante;
			$consulta = parent::consulta("SELECT * FROM sw_estudiante_periodo_lectivo WHERE id_estudiante = $id_estudiante AND id_periodo_lectivo = $id_periodo_lectivo");
			return (parent::num_rows($consulta) > 0);
		} else {
			return false;
		}
	...
	function verificarEstudianteMatriculado()
	{
		$consulta = parent::consulta("SELECT COUNT(*) AS contador FROM sw_estudiante_periodo_lectivo WHERE id_estudiante = " . $this->code . " AND id_periodo_lectivo = " . $this->id_periodo_lectivo);
		$registro = parent::fetch_assoc($consulta);
		return ($registro["contador"] > 0);
	}
	...
	function determinarCedulaRepetida()
	{
		$consulta = parent::consulta("SELECT * FROM sw_estudiante WHERE es_cedula = '" . $this->es_cedula . "'");
		$num_registros = parent::num_rows($consulta);
		$cadena = "";
		if ($num_registros > 0) {
			$cadena = "Ya existe ese numero de cedula en la base de datos...";
		}
		return $cadena;
	}

	function buscarEstudiantes($patron)
	{
		$query = parent::consulta("SELECT id_estudiante, CONCAT(es_apellidos,' ',es_nombres) AS nombre FROM sw_estudiante WHERE es_nombres LIKE '%$patron%' OR es_apellidos LIKE '%$patron%'");
		$resultado = array();
		while ($row = parent::fetch_object($query)) {
			$datos = array(
	...
	function obtenerHistorialEstudiante()
	{
		$consulta = parent::consulta("SELECT e.id_estudiante, 
									   ...
									   WHERE e.id_estudiante = ep.id_estudiante 
										 ...
										 AND pe.id_periodo_lectivo = ep.id_periodo_lectivo 
										 AND m.id_modalidad = pe.id_modalidad 
										 AND e.id_estudiante = $this->code 
										 AND ep.id_periodo_lectivo <= $this->id_periodo_lectivo - 1 
										 ORDER BY pe_fecha_inicio, pe_fecha_fin");
		$num_registros = parent::num_rows($consulta);
		$cadena = "";
		if ($num_registros > 0) {
			while ($estudiante = parent::fetch_assoc($consulta)) {
				...
				$modalidad = $estudiante["mo_nombre"];
				$periodo_lectivo = $estudiante["pe_anio_inicio"] . " - " . $estudiante["pe_anio_fin"];
				$especialidad = $estudiante["es_abreviatura"];
				...
				$cadena .= "<td>$modalidad</td>\n";
				$cadena .= "<td>$periodo_lectivo</td>\n";
				$cadena .= "<td>$curso ($especialidad)</td>\n";
				$cadena .= "<td>$paralelo</td>\n";
				$cadena .= "<td>$c_aprobado</td>\n";
				$cadena .= "</tr>\n";
			}
		} else {
		}
	...
	{
		$consulta = parent::consulta("SELECT e.* FROM sw_estudiante e WHERE id_estudiante = " . $this->code);
	...
	{
		$consulta = parent::consulta("SELECT es_figura, cu_nombre, pa_nombre, jo_nombre FROM sw_estudiante e, sw_estudiante_periodo_lectivo ep, sw_especialidad es, sw_curso c, sw_paralelo p, sw_jornada j WHERE ep.id_estudiante = e.id_estudiante AND ep.id_paralelo = p.id_paralelo AND p.id_curso = c.id_curso AND c.id_especialidad = es.id_especialidad AND j.id_jornada = p.id_jornada AND e.id_estudiante = $id_estudiante AND ep.id_periodo_lectivo = $id_periodo_lectivo");
		$estudiante = parent::fetch_assoc($consulta);
		return $estudiante["cu_nombre"] . " " . $estudiante["pa_nombre"] . " " . $estudiante["es_figura"] . " - " . $estudiante["jo_nombre"];
	}
	...
	function actualizarEstadoRetirado($estado_retirado)
	{
		...
		else echo "Estado de retirado del estudiante no pudo actualizarse. Error: " . mysqli_error($this->conexion);
	}
		...
		while ($asignatura = parent::fetch_assoc($asignaturas)) {
			...
			if ($num_total_registros > 0) {
				...
				while ($periodo = parent::fetch_assoc($periodo_evaluacion)) {
					...
					$ponderado_aportes = 0.8 * $promedio_aportes;
					$ponderado_examen = 0.2 * $examen_quimestral;
					$calificacion_quimestral = self::truncateFloat($ponderado_aportes + $ponderado_examen, 2);
					$suma_periodos += $calificacion_quimestral;
					//echo $suma_periodos . "<br>";
					$cadena .= "<td width=\"60px\" align=\"left\">" . number_format(self::truncateFloat($calificacion_quimestral, 2), 2) . "</td>\n";
					//$cadena .= "<td width=\"60px\" align=\"left\">".number_format($calificacion_quimestral,2)."</td>\n";
				} // fin while $periodo_evaluacion
			} // if($num_total_registros>0)
			// Calculo la suma y el promedio de los dos quimestres
			$promedio_periodos = self::truncateFloat($suma_periodos / $contador_periodos, 2);
			$promedio_final = $promedio_periodos;
			$examen_supletorio = 0;
			$examen_remedial = 0;
			$examen_de_gracia = 0;
			...
			$cadena .= "<td width=\"60px\" align=\"left\">" . number_format(self::truncateFloat($suma_periodos, 2), 2) . "</td>\n"; // Suma
			$cadena .= "<td width=\"60px\" align=\"left\">" . number_format($promedio_periodos, 2) . "</td>\n"; // Prom. Quim.
			$cadena .= "<td width=\"60px\" align=\"left\">" . number_format($examen_supletorio, 2) . "</td>\n"; // Supletorio
			$cadena .= "<td width=\"60px\" align=\"left\">" . number_format($examen_remedial, 2) . "</td>\n"; // Remedial
			$cadena .= "<td width=\"60px\" align=\"left\">" . number_format($examen_de_gracia, 2) . "</td>\n"; // Gracia
		...
		while ($asignatura = parent::fetch_assoc($asignaturas)) {
			...
			if ($num_total_registros > 0) {
				...
				while ($aporte = parent::fetch_assoc($aporte_evaluacion)) {
					...
					$promedio = $suma_rubricas / $contador_rubricas;
					if ($contador_aportes <= $num_total_registros - 1) {
						$cadena .= "<td width=\"60px\" align=\"right\">" . substr($promedio, 0, strpos($promedio, '.') + 3) . "</td>";
						$suma_promedios += $promedio;
					} else {
						$examen_quimestral = $promedio;
					}
				...
				$ponderado_examen = 0.2 * $examen_quimestral;
				$calificacion_quimestral = $ponderado_aportes + $ponderado_examen;
				$cadena .= "<td width=\"60px\" align=\"right\">" . substr($promedio_aportes, 0, strpos($promedio_aportes, '.') + 3) . "</td>";
				$cadena .= "<td width=\"60px\" align=\"right\">" . substr($ponderado_aportes, 0, strpos($ponderado_aportes, '.') + 3) . "</td>";
				$cadena .= "<td width=\"60px\" align=\"right\">" . number_format($examen_quimestral, 2) . "</td>";
				$cadena .= "<td width=\"60px\" align=\"right\">" . substr($ponderado_examen, 0, strpos($ponderado_examen, '.') + 3) . "</td>";
				$cadena .= "<td width=\"60px\" align=\"right\">" . substr($calificacion_quimestral, 0, strpos($calificacion_quimestral, '.') + 3) . "</td>";
			}
			//*******************************************************************
			$cadena .= "<td width=\"*\">&nbsp;</td>\n"; // Esto es para igualar el espaciado entre columnas
			$cadena .= "</tr>\n";
	...
	{
		$consulta = parent::consulta("SELECT e.id_estudiante, es_apellidos, es_nombres, es_retirado, es_cedula, es_fec_nacim, dg_nombre, nro_matricula, pe_estado FROM sw_estudiante e, sw_estudiante_periodo_lectivo ep, sw_periodo_lectivo p, sw_def_genero dg WHERE e.id_estudiante = ep.id_estudiante AND p.id_periodo_lectivo = ep.id_periodo_lectivo AND dg.id_def_genero = e.id_def_genero AND ep.id_periodo_lectivo = $this->id_periodo_lectivo AND id_paralelo = $this->id_paralelo AND activo = 1 ORDER BY es_apellidos, es_nombres ASC");
		...
		if ($num_total_registros > 0) {
			...
			while ($estudiantes = parent::fetch_assoc($consulta)) {
				...
				$estado = $estudiantes["pe_estado"];
				$disabled = ($estado == "T") ? "disabled" : "";
				$cedula = $estudiantes["es_cedula"];
				$genero = $estudiantes["dg_nombre"];
				$nro_matricula = $estudiantes["nro_matricula"];
				$fec_nacim = $estudiantes["es_fec_nacim"];
				...
				$cadena .= "<a href='javascript:;' class='btn btn-warning btn-sm item-edit' onclick='editarEstudiante($codigo)' title='Editar'><span class='fa fa-pencil'></span></a>\n";
				if ($estado != "T") {
					$cadena .= "<a href='javascript:;' class='btn btn-danger btn-sm item-delete' onclick='quitarEstudiante($codigo)' title='Quitar'><span class='fa fa-remove'></span></a>\n";
				}
				$cadena .= "<a href='javascript:;' class='btn btn-info btn-sm item-info' onclick='certificadoMatricula($codigo)' title='Certificado'><span class='fa fa-file-text'></span></a>\n";
				$cadena .= "</div>\n";
				$cadena .= "</td>\n";
				$cadena .= "</tr>\n";
			}
	...
	{
		$consulta = parent::consulta("SELECT e.id_estudiante, es_apellidos, es_nombres, es_retirado, es_cedula FROM sw_estudiante e, sw_estudiante_periodo_lectivo p WHERE e.id_estudiante = p.id_estudiante AND id_periodo_lectivo = " . $this->id_periodo_lectivo . " AND id_paralelo = " . $this->id_paralelo . " AND activo = 1 ORDER BY es_apellidos, es_nombres ASC");
		$num_total_registros = parent::num_rows($consulta);
		...
		$cadena .= "<th colspan=\"2\" class=\"text-center\">Acciones</th>\n";
		$cadena .= "</tr>\n";
		$cadena .= "</thead>\n";
		$cadena .= "<tbody>\n";
		if ($num_total_registros > 0) {
			...
			while ($estudiantes = parent::fetch_assoc($consulta)) {
				...
				$aprueba = $this->es_promocionado($codigo, $this->id_periodo_lectivo, $this->id_paralelo) ? 'APRUEBA' : 'NO APRUEBA';
				$cadena .= "<td width=\"16%\" align=\"left\">$pro</td>\n";
				$cadena .= "<td width=\"8%\" align=\"left\">$aprueba</td>\n";
				$cadena .= "<td width=\"18%\"><a href=\"#\"class=\"btn btn-success btn-sm\" onclick=\"seleccionarEstudiante(" . $codigo . ")\"><i class=\"fa fa-fw fa-print\"></i> Certificado</a></td>\n";
				$cadena .= "</tr>\n";
			}
		} else {
			$cadena .= "<tr>\n";
	...
	{
		// Primero verifica si no se duplica el número de cédula en caso de existir...
		$qry = "SELECT * FROM sw_estudiante WHERE es_cedula = '" . $this->es_cedula . "'";
		$consulta = parent::consulta($qry);
		...
		if ($num_rows > 0) {
			$datos = array(
				'ok' => false,
				'titulo' => "Error!",
				'mensaje' => "Ya existe ese numero de cedula en la base de datos...",
				'estado' => "error",
				'id_estudiante' => 0
			);
		} else {
			// Luego calculo cual es el maximo numero de matricula
			$qry = "SELECT MAX(nro_matricula) AS max_nro_matricula FROM sw_estudiante_periodo_lectivo WHERE id_periodo_lectivo = $this->id_periodo_lectivo";
			$consulta = parent::consulta($qry);
			if (!$consulta) {
				$mensaje = "No se pudo obtener el maximo numero de matricula...Error: " . mysqli_error($this->conexion);
				$datos = array(
					'titulo' => "Error!",
					'mensaje' => $mensaje,
					'estado' => "error",
				...
				if ($num_rows > 0) {
					...
					$max_nro_matricula = $maximo->max_nro_matricula + 1;
				} else {
					$max_nro_matricula = 1;
				}
				...
				$qry = "INSERT INTO sw_estudiante (
				        ...
				        es_email, es_nombre_completo, 
					    es_direccion, es_sector, es_telefono, es_fec_nacim,
						id_tipo_documento, id_def_genero, id_def_nacionalidad) VALUES (";
				...
				$qry .= "'" . $this->es_fec_nacim . "',";
				$qry .= $this->id_tipo_documento . ",";
				$qry .= $this->id_def_genero . ",";
				$qry .= $this->id_def_nacionalidad . ")";
				//echo $qry;
				$consulta = parent::consulta($qry);
				if (!$consulta) {
					$mensaje = "No se pudo insertar el Estudiante...Error: " . mysqli_error($this->conexion);
					$datos = array(
						'titulo' => "Error!",
						'mensaje' => $mensaje,
						'estado' => "error",
					...
					$qry = "INSERT INTO sw_estudiante_periodo_lectivo (id_estudiante, id_periodo_lectivo, id_paralelo, es_estado, es_retirado, nro_matricula, activo) VALUES (";
					$qry .= $id_estudiante . ",";
					$qry .= $this->id_periodo_lectivo . ",";
					$qry .= $this->id_paralelo . ",'N','N',";
					$qry .= $max_nro_matricula . ", 1)";
					$consulta = parent::consulta($qry);
	...
	function insertarEstudianteSeleccionado($nombre_estudiante)
	{
		// Falta comprobar que no se ingrese un estudiante de un paralelo que no corresponde...
		// Aqui obtengo el maximo numero secuencial de matricula del año lectivo en curso...
		$qry = "SELECT MAX(nro_matricula) AS max_nro_matricula FROM sw_estudiante_periodo_lectivo WHERE id_periodo_lectivo = " . $this->id_periodo_lectivo;
		$consulta = parent::consulta($qry);
		if (!$consulta) {
			$mensaje = "No se pudo calcular el maximo numero de matricula...Error: " . mysqli_error($this->conexion);
		} else {
			$num_rows = parent::num_rows($consulta);
			if ($num_rows > 0) {
				$maximo = parent::fetch_object($consulta);
			...
			$qry = "INSERT INTO sw_estudiante_periodo_lectivo (id_estudiante, id_periodo_lectivo, id_paralelo, es_estado, es_retirado, activo, nro_matricula) VALUES (";
			$qry .= $this->code . ",";
			$qry .= $this->id_periodo_lectivo . ",";
			$qry .= $this->id_paralelo . ",'N','N',1,";
			$qry .= $max_nro_matricula . ")";
			$consulta = parent::consulta($qry);
			$mensaje = "Estudiante " . $nombre_estudiante . " insertado exitosamente...";
			if (!$consulta)
				$mensaje = "No se pudo insertar el Estudiante...Error: " . mysqli_error($this->conexion);
		}
	...
	function actualizarEstudiante()
	{
		...
		$qry .= "es_nombre_completo = '" . $this->es_apellidos . " " . $this->es_nombres . "',";
		...
		$qry .= "es_fec_nacim = '" . $this->es_fec_nacim . "',";
		$qry .= "id_tipo_documento = " . $this->id_tipo_documento . ",";
		$qry .= "id_def_genero = " . $this->id_def_genero . ",";
		$qry .= "id_def_nacionalidad = " . $this->id_def_nacionalidad;
		$qry .= " WHERE id_estudiante = " . $this->code;
		//echo $qry;
		$consulta = parent::consulta($qry);
		$qry = "SELECT id_paralelo FROM sw_estudiante_periodo_lectivo WHERE id_estudiante = " . $this->code . " AND id_periodo_lectivo = " . $this->id_periodo_lectivo;
		$consulta = parent::consulta($qry);
		$resultado = parent::fetch_assoc($consulta);
		$id_paralelo_actual = $resultado["id_paralelo"];
		if ($this->id_paralelo != "" && $this->id_paralelo != $id_paralelo_actual) {
			$qry = "SELECT id_periodo_lectivo FROM sw_paralelo WHERE id_paralelo = " . $this->id_paralelo;
			$consulta = parent::consulta($qry);
			$registro = parent::fetch_object($consulta);
			$id_periodo_lectivo_nuevo = $registro->id_periodo_lectivo;
			$qry = "UPDATE sw_estudiante_periodo_lectivo SET ";
			$qry .= "id_paralelo = " . $this->id_paralelo;
			$qry .= ", id_periodo_lectivo = " . $id_periodo_lectivo_nuevo;
			$qry .= " WHERE id_estudiante = " . $this->code;
			$qry .= " AND id_periodo_lectivo = " . $this->id_periodo_lectivo;
			$consulta = parent::consulta($qry);
			//Actualiza las calificaciones del estudiante
			$qry = "UPDATE sw_rubrica_estudiante SET id_paralelo = " . $this->id_paralelo . " WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
			//Actualiza las calificaciones de comportamiento ingresadas por los docentes
			$qry = "UPDATE sw_calificacion_comportamiento SET id_paralelo = " . $this->id_paralelo . " WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
			//Actualiza las calificaciones de comportamiento ingresadas por el tutor
			$qry = "UPDATE sw_comportamiento_tutor SET id_paralelo = " . $this->id_paralelo . " WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
			$qry = "UPDATE sw_asistencia_estudiante SET id_paralelo = " . $this->id_paralelo . " WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
			$qry = "UPDATE sw_asistencia_tutor SET id_paralelo = " . $this->id_paralelo . " WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
			//Elimino las asistencias registradas por parte del docente y del tutor
			$qry = "DELETE FROM sw_asistencia_estudiante WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
			$qry = "DELETE FROM sw_asistencia_tutor WHERE id_estudiante = " . $this->code . " AND id_paralelo = $id_paralelo_actual";
			$consulta = parent::consulta($qry);
		}
	...
	function insertarRepresentante()
	{
		...
		$consulta = parent::consulta($qry);
		if (!$consulta)
			$mensaje = "No se pudo insertar el Representante...Error: " . mysqli_error($this->conexion);
		else {
	...
	function actualizarRepresentante()
	{
		...
		$consulta = parent::consulta($qry);
		if (!$consulta)
			$mensaje = "No se pudo actualizar el Representante...Error: " . mysqli_error($this->conexion);
		else {
	...
	function eliminarEstudiante()
	{
		...
		// 	$consulta = parent::consulta($qry);
		// 	if (!$consulta) {
		// 		$mensaje = "No se pudo eliminar el Estudiante...Error: " . mysqli_error($this->conexion);
		// 	} else {
		...
		try {
			...
			$data = [
				...
				'mensaje' => 'Estudiante eliminado exitosamente de la Base de Datos.'
			];
		} catch (Exception $e) {
			$data = [
				'icono' => 'error',
				'titulo' => 'Operación fallida.',
				'mensaje' => 'El estudiante no pudo ser eliminado de la Base de Datos. Error: ' . $e->getMessage()
			];
		}
	...
	function quitarEstudiante()
	{
		...
		$consulta = parent::consulta($qry);
		if (!$consulta) {
			$mensaje = "No se pudo eliminar el representante asociado...Error: " . mysqli_error($this->conexion);
			return $mensaje;
		}
		$qry = "DELETE FROM sw_rubrica_estudiante WHERE id_estudiante=". $this->code ." AND id_paralelo=". $this->id_paralelo;
		$consulta = parent::consulta($qry);
		if (!$consulta) {
			$mensaje = "No se pudo eliminar las calificaciones asociadas...Error: " . mysqli_error($this->conexion);
			return $mensaje;
		}
 ...
 */
		$qry = "UPDATE sw_estudiante_periodo_lectivo SET activo = 0 WHERE id_estudiante = $this->code AND id_periodo_lectivo = $this->id_periodo_lectivo AND id_paralelo = $this->id_paralelo";
		$consulta = parent::consulta($qry);
		if (!$consulta) {
			$icono = "error";
			$titulo = "Oops! Ocurrió un error inesperado...";
			$mensaje = "No se pudo des-matricular al Estudiante...Error: " . mysqli_error($this->conexion);
		} else {
			$icono = "success";
			$titulo = "Operación exitosa.";
	...
	function getNumeroEstudiantesPorParalelo($id_periodo_lectivo, $id_jornada)
	{
	...
	{
		$consulta = parent::consulta("SELECT COUNT(*) AS num_registros FROM sw_estudiante e, sw_estudiante_periodo_lectivo p WHERE e.id_estudiante = p.id_estudiante AND id_periodo_lectivo = $this->id_periodo_lectivo AND id_paralelo = $this->id_paralelo AND activo = 1 AND es_retirado = 'N'");
		return json_encode(parent::fetch_assoc($consulta));
	}
