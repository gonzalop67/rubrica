...
class rubricas_evaluacion extends MySQL
{
	...
	function cargarRubricasEvaluacion()
	{
		...
		if ($num_total_registros > 0) {
			while ($rubricas_evaluacion = parent::fetch_assoc($consulta)) {
				...
				$cadena .= "<button class=\"btn btn-warning\" onclick=\"obtenerDatos(" . $id . ")\" data-toggle=\"modal\" data-target=\"#editarInsumoEvaluacionModal\" title=\"Editar\"><span class=\"fa fa-pencil\"></span></button>\n";
				$cadena .= "<button class=\"btn btn-danger\" onclick=\"eliminarInsumoEvaluacion(" . $id . ")\" title=\"Eliminar\"><span class=\"fa fa-trash\"></span></button>\n";
				$cadena .= "</div>";
				$cadena .= "</td>\n";
	...
	{
		try {
			$qry = "INSERT INTO sw_rubrica_evaluacion (id_aporte_evaluacion, id_tipo_asignatura, ru_nombre, ru_abreviatura, ru_ponderacion, ru_descripcion) VALUES (";
			$qry .= $this->id_aporte_evaluacion . ",";
			$qry .= $this->id_tipo_asignatura . ",";
			$qry .= "'" . $this->ru_nombre . "',";
			$qry .= "'" . $this->ru_abreviatura . "',";
			$qry .= $this->ru_ponderacion . ",";
			$qry .= "'" . $this->ru_descripcion . "')";

			$consulta = parent::consulta($qry);

			$datos = array(
				'titulo' => 'Yes!',
				'mensaje' => 'Se ha insertado exitosamente el Insumo de Evaluación.',
				'estado' => 'success'
			);
		} catch (Exception $e) {
			$datos = array(
				'titulo' => 'Oops!',
				'mensaje' => 'No se insertó exitosamente el Insumo de Evaluación. Error: ' . $e->getMessage(),
				'estado' => 'success'
			);
		}
	...
	function actualizarRubricaEvaluacion()
	{
		try {
			...
			$qry .= "ru_ponderacion = " . $this->ru_ponderacion . ",";
			$qry .= "ru_descripcion = '" . $this->ru_descripcion . "'";
			...
			$datos = array(
				'titulo' => 'Yes!',
				'mensaje' => 'Se ha actualizado exitosamente el Insumo de Evaluación.',
				'estado' => 'success'
			);
		} catch (Exception $e) {
			$datos = array(
	...
	function eliminarRubricaEvaluacion()
	{
		...
		if (parent::num_rows($consulta) > 0) {
			...
			try {
				...
				$datos = array(
					'titulo' => 'Yes!',
					'mensaje' => 'Se ha eliminado con éxito el insumo de evaluación.',
					'estado' => 'success'
				);
			} catch (Exception $e) {
				$datos = array(
					'titulo' => 'Oops!',
					'mensaje' => 'No se pudo eliminar el insumo de evaluación. Error: ' . $e->getMessage(),
					'estado' => 'error'
				);
			}
		}
	...
	function eliminarRubricaProyecto()
	{
		...
		if (parent::num_rows($consulta) > 0) {
			...
			if (!$consulta)
				$mensaje = "No se pudo eliminar la R&uacute;brica de Proyecto...Error: " . mysqli_error($this->conexion);
		}
		return $mensaje;
	}
	...
	function insertarRubricaEstudianteComportamiento()
	{
		...
		$qry .= "id_asignatura = " . $this->id_asignatura . ",";
		$qry .= "id_aporte_evaluacion = " . $this->id_aporte_evaluacion . ",";
		$qry .= "co_cualitativa = '" . $this->re_calificacion . "'";
		$consulta = parent::consulta($qry);
		...
		if (!$consulta) {
			...
			$qry = "SELECT ec_correlativa FROM sw_escala_comportamiento WHERE ec_equivalencia = '" . $this->re_calificacion . "'";
			$consulta = parent::consulta($qry);
			if (!$consulta) {
				$mensaje = "No se pudo realizar la consulta... Error: " . mysqli_error($this->conexion);
				return $mensaje;
			} else {
	...
	function insertarRubricaCualitativa()
	{
		...
		$qry .= "id_asignatura = " . $this->id_asignatura . ",";
		$qry .= "id_aporte_evaluacion = " . $this->id_aporte_evaluacion . ",";
		$qry .= "rc_calificacion = '" . $this->re_calificacion . "'";
		// return $qry;
		$consulta = parent::consulta($qry);
	...
	function eliminarRubricaEstudiante()
	{
	...
	function actualizarRubricaPersonalizada()
	{
		...
		if ($consulta)
			...
			$mensaje = "No se pudo actualizar la r&uacute;brica personalizada... Error: " . mysqli_error($this->conexion);
		return $mensaje;
	}
	...
	function actualizarCalificacionErronea()
	{
		...
		if ($consulta)
			...
			$mensaje = "No se pudo actualizar la r&uacute;brica... Error: " . mysqli_error($this->conexion);
		return $mensaje;
	}
