...
class menus extends MySQL
{
	...
	function eliminarMenu()
	{
		...
		if ($menu->num_submenus > 0)
			...
			$qry = "SELECT mnu_padre, mnu_orden FROM sw_menu WHERE id_menu = "	. $this->code;
			$consulta = parent::consulta($qry);
			$menu = parent::fetch_object($consulta);
			$mnu_orden = $menu->mnu_orden;
			$mnu_padre = $menu->mnu_padre;
			$menus = parent::consulta("SELECT id_menu FROM `sw_menu` WHERE mnu_padre = $mnu_padre AND mnu_orden > $mnu_orden");
			while ($menu = parent::fetch_object($menus)) {
				$consulta = parent::consulta("UPDATE sw_menu SET mnu_orden = mnu_orden - 1 WHERE id_menu = $menu->id_menu");
			}
			$qry = "DELETE FROM sw_menu WHERE id_menu=". $this->code;
			$consulta = parent::consulta($qry);
			$mensaje = "Men&uacute; eliminado exitosamente...";
			if (!$consulta)
				$mensaje = "No se pudo eliminar el men&uacute;...Error: " . mysqli_error($this->conexion);
		}
		return $mensaje;
	}
	...
	function subirMenuPerfil()
	{
		...
		if (!$consulta)
			$mensaje = "No se pudo \"subir\" el Menu...Error: " . mysqli_error($this->conexion);
	...
	function subirSubmenu()
	{
		...
		if (!$consulta)
			$mensaje = "No se pudo \"subir\" del Submenu...Error: " . mysqli_error($this->conexion);
	...
	function bajarSubmenu()
	{
		...
		$mensaje = "Submenu \"bajado\" exitosamente...";

		if (!$consulta)
			$mensaje = "No se pudo \"bajar\" del Submenu...Error: " . mysqli_error($this->conexion);
	...
	function bajarMenuPerfil()
	{
		...
		if (!$consulta)
			$mensaje = "No se pudo \"bajar\" el Menu...Error: " . mysqli_error($this->conexion);
