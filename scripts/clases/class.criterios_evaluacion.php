...
class criterios_evaluacion extends MySQL
{
	...
	function insertarCriterioEvaluacion()
	{
		...
		if($suma_ponderacion->suma + $this->cr_ponderacion > 1)
			...
			if (!$consulta)
				$mensaje = "No se pudo insertar la Criterio de Evaluaci&oacute;n...Error: " . mysqli_error($this->conexion);
		}
	...
	function insertarCriterioPersonalizado()
	{
		...
		if($suma_ponderacion->suma + $this->cr_ponderacion > 1)
			...
			if (!$consulta)
				$mensaje = "No se pudo insertar la Criterio de Evaluaci&oacute;n Personalizado...Error: " . mysqli_error($this->conexion);
		}
	...
	function actualizarCriterioEvaluacion()
	{
		...
		if (!$consulta)
			$mensaje = "No se pudo actualizar el Criterio de Evaluaci&oacute;n...Error: " . mysqli_error($this->conexion);
		return $mensaje;
	}
	...
	function eliminarCriterioEvaluacion()
	{
		...
		$consulta = parent::consulta($qry) or die(mysqli_error($this->conexion));
		...
		if (!$consulta)
			$mensaje = "No se pudo eliminar el Criterio de Evaluaci&oacute;n...Error: " . mysqli_error($this->conexion);
		return $mensaje;
	}
	...
	function obtenerRubricaPersonalizada($id_estudiante, $id_asignatura, $id_rubrica_evaluacion, $id_usuario, $id_paralelo)
	{
		...
		if($num_total_registros>0) {
			...
			while($criterios_evaluacion = parent::fetch_assoc($consulta))
			{
				// Obtenci�n de los puntajes si existen
				...
				$resultado = parent::consulta($qry) or die(mysqli_error($this->conexion));
				if($resultado) {
