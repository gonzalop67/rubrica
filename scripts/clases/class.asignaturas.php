...
class asignaturas extends MySQL
{
	...
	function existeAsignatura($campo, $valor, $id_area)
	{
		$consulta = parent::consulta("SELECT * FROM sw_asignatura WHERE $campo = '$valor' AND id_area = $id_area");
	...
	function cargarAsignaturas()
	{
		...
		if ($num_total_registros > 0) {
			...
			while ($asignatura = parent::fetch_assoc($consulta)) {
				...
				$cadena .= "<button class=\"btn btn-warning\" onclick=\"obtenerDatos(" . $id . ")\" data-toggle=\"modal\" data-target=\"#editarAsignaturaModal\" title=\"Editar\"><span class=\"fa fa-pencil\"></span></button>\n";
				$cadena .= "<button class=\"btn btn-danger\" onclick=\"eliminarAsignatura(" . $id . ")\" title=\"Eliminar\"><span class=\"fa fa-trash\"></span></button>\n";
				$cadena .= "</div>\n";
				$cadena .= "</td>\n";
	...
	function buscarAsignaturas($busqueda)
	{
		$consulta = parent::consulta(
			"SELECT id_asignatura, 
			  ...
			  WHERE ar.id_area = a.id_area 
			    AND (as_nombre LIKE '%$busqueda%' 
			     OR as_abreviatura LIKE '%$busqueda%'
				 OR ar_nombre LIKE '%$busqueda%')
	...
	function cargarComboAsignaturasDocente()
	{
		$consulta = parent::consulta("SELECT a.id_asignatura, 
											 as_nombre, 
											 c.id_curso,
											 d.id_paralelo,
											 d.id_asignatura,
		...
		if ($num_total_registros > 0) {
			while ($paralelo = parent::fetch_assoc($consulta)) {
				$id_asignatura = $paralelo["id_asignatura"];
				$as_nombre = $paralelo["as_nombre"];
				$id_paralelo = $paralelo["id_paralelo"];
				$name = $as_nombre . " - " . $paralelo["cu_nombre"] . " " . $paralelo["pa_nombre"] . " - " . $paralelo["es_figura"];
				$cadena .= "<option value=\"$id_asignatura*$id_paralelo\">$name</option>";
			}
		}
		return $cadena;
	}
	...
	function listarEscalaCalificacionesAnual($id_periodo_lectivo, $id_paralelo, $id_asignatura)
	{
		...
		$datos = array();
		$estudiantes = parent::consulta("SELECT id_estudiante FROM sw_estudiante_periodo_lectivo WHERE id_paralelo = $id_paralelo AND es_retirado = 'N' AND activo = 1");
		...
		if ($num_total_estudiantes > 0) {
			while ($estudiante = parent::fetch_assoc($estudiantes)) {
				...
				$query = parent::consulta("SELECT calcular_promedio_periodo_lectivo($id_periodo_lectivo, $id_estudiante, $id_paralelo, $id_asignatura) AS calificacion");
	...
	function listarEscalaCalificacionesQuimestrales($id_periodo_evaluacion, $id_paralelo, $id_asignatura, $id_periodo_lectivo)
	{
		...
		// Aqui va el codigo para calcular el promedio del aporte de cada estudiante
		$datos = array();
		$estudiantes = parent::consulta("SELECT id_estudiante FROM sw_estudiante_periodo_lectivo WHERE id_paralelo = $id_paralelo AND id_periodo_lectivo = $id_periodo_lectivo AND es_retirado = 'N' AND activo = 1");
		$num_total_estudiantes = parent::num_rows($estudiantes);
		if ($num_total_estudiantes > 0) {
			while ($estudiante = parent::fetch_assoc($estudiantes)) {
				$id_estudiante = $estudiante["id_estudiante"];
				$query = parent::consulta("SELECT calcular_promedio_sub_periodo($id_periodo_evaluacion,$id_estudiante,$id_paralelo,$id_asignatura) AS calificacion");
				$calificacion = parent::fetch_assoc($query);
	...
	function insertarAsignatura()
	{
		...
		if ($this->existeAsignatura('as_nombre', $this->as_nombre, $this->id_area)) {
			$datos = [
				'titulo' => "¡Error!",
				'mensaje' => "Ya existe una asignatura con ese nombre en la Base de Datos.",
				'estado' => 'error'
			];
		} else {
			try {
	...
	function actualizarAsignatura()
	{
		...
		$nombreActual = $registro["as_nombre"];
		$abreviaturaActual = $registro["as_abreviatura"];

		if ($nombreActual != $this->as_nombre && $this->existeAsignatura('as_nombre', $this->as_nombre, $this->id_area)) {
			$datos = [
				'titulo' => "¡Error!",
				'mensaje' => "Ya existe una asignatura con ese nombre en la Base de Datos.",
				'estado' => 'error'
			];
		} else if ($abreviaturaActual != $this->as_abreviatura && $this->existeAsignatura('as_abreviatura', $this->as_abreviatura, $this->id_area)) {
			$datos = [
				'titulo' => "¡Error!",
				'mensaje' => "Ya existe una asignatura con esa abreviatura en la Base de Datos.",
				'estado' => 'error'
			];
		} else {
			try {
	...
	function mostrarTitulosAsignaturas($tipo_reporte, $alineacion)
	{
		...
		$consulta = parent::consulta("SELECT as_abreviatura FROM sw_asignatura a, sw_asignatura_curso ac, sw_paralelo p WHERE a.id_asignatura = ac.id_asignatura AND p.id_curso = ac.id_curso AND a.id_tipo_asignatura = 1 AND p.id_paralelo = " . $this->code . " ORDER BY ac_orden");
		$mensaje = "<table id=\"titulos_asignaturas\" class=\"fuente8\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">\n";
		$mensaje .= "<tr>\n";
		$num_total_registros = parent::num_rows($consulta);
		if ($num_total_registros > 0) {
