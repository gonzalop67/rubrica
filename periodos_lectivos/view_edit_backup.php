...
//Obtener subniveles de educación
$niveles = $db->consulta("SELECT * FROM sw_sub_nivel_educacion ORDER BY orden");
//Obtener subperiodos de evaluación
$subperiodos = $db->consulta("SELECT * FROM sw_sub_periodo_evaluacion ORDER BY pe_orden");
//Recuperar los subniveles de educación asociados
$consulta = $db->consulta("SELECT * FROM sw_periodo_nivel WHERE id_periodo_lectivo = $_GET[id_periodo_lectivo]");
$niveles_educacion = array();
while ($v = $db->fetch_object($consulta)) {
    array_push($niveles_educacion, $v->id_nivel_educacion);
}
//Recuperar los subperiodos de evaluación asociados
$consulta = $db->consulta("SELECT * FROM sw_periodo_lectivo_sub_periodo WHERE id_periodo_lectivo = $_GET[id_periodo_lectivo]");
$subperiodos_evaluacion = array();
while ($v = $db->fetch_object($consulta)) {
    array_push($subperiodos_evaluacion, $v->id_sub_periodo_evaluacion);
}
?>
                            ...
                            <form id="frm-periodo-lectivo" action="periodos_lectivos/actualizar_periodo_lectivo.php" method="post">
                                ...
                                <div class="row mb-3">
                                    <div id="subniveles" class="col-lg-6">
                                        ...
                                        <?php
                                        while ($v = $db->fetch_object($niveles)) {
                                        ?>
                                            <br>
                                            <input type="checkbox" name="niveles[]"
                                                value="<?= $v->id_nivel_educacion ?>"
                                                <?= in_array($v->id_nivel_educacion, $niveles_educacion) ? 'checked' : '' ?>> <?= $v->nombre ?>
                                        <?php
                                        }
                                        ?>
                                    </div>
                                    <div id="subperiodos" class="col-lg-6">
                                        ...
                                        <?php
                                        while ($v = $db->fetch_object($subperiodos)) {
                                        ?>
                                            <br>
                                            <input type="checkbox" name="sub_periodos[]"
                                                value="<?= $v->id_sub_periodo_evaluacion ?>"
                                                <?= in_array($v->id_sub_periodo_evaluacion, $subperiodos_evaluacion) ? 'checked' : '' ?>> <?= $v->pe_nombre ?>
                                        <?php
                                        }
                                        ?>
                                    </div>
