<?php
	...
	// Primero recupero quien inserta el comportamiento

	$consulta = $db->consulta("SELECT quien_inserta_comp FROM sw_curso c, sw_paralelo p WHERE c.id_curso = p.id_curso AND id_paralelo = $id_paralelo");
	...
	if($num_total_registros>0)
	{
		...
		while($paralelo = $db->fetch_assoc($consulta))
		{
			...
			if ($quien_inserta_comportamiento == 0) {  // Comportamiento ingresado por los docentes
				...
				if ($total_asignaturas > 0)
				{
					...
					$pdf->Cell(36,6,$comp_docentes,1,0,'C');
			...
			} else { // Comportamiento ingresado por los tutores
				$query = $db->consulta("SELECT co_calificacion FROM sw_comportamiento_tutor WHERE id_paralelo = $id_paralelo AND id_estudiante = $id_estudiante AND id_aporte_evaluacion = $id_aporte_evaluacion");
				$resultado = $db->fetch_assoc($query);
				$comportamiento = $resultado["co_calificacion"];
				$pdf->Cell(36,6,$comportamiento,1,0,'C');
			}
