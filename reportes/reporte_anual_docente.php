<?php
	...
	if($num_total_registros>0)
	{
		...
		while($estudiante = $db->fetch_assoc($consulta))
		{
			...
			$terminacion = ($estudiante["dg_abreviatura"] == "M") ? "O" : "A";
			...
			if($num_total_registros>0)
			{
				...
				while($periodo = $db->fetch_assoc($periodo_evaluacion))
				{
						...
						while($aporte = $db->fetch_assoc($aporte_evaluacion))
						{
							...
							if($total_rubricas>0)
							{
								...
								while($rubricas = $db->fetch_assoc($rubrica_evaluacion))
								{
									...
									$query = $db->consulta("SELECT re_calificacion FROM sw_rubrica_estudiante WHERE id_estudiante = $id_estudiante AND id_paralelo = $id_paralelo AND id_asignatura = $id_asignatura AND id_rubrica_personalizada = $id_rubrica_evaluacion");
							...
							$promedio = $paralelo->truncar($suma_rubricas / $contador_rubricas,2);
					...
					$calificacion_quimestral = $paralelo->truncar($ponderado_aportes + $ponderado_examen,2);
			...
			$promedio_periodos = $paralelo->truncar($suma_periodos / $contador_periodos,2);
