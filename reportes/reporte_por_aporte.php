<?php
	...
	$db = new MySQL();
	//Primero consultamos si se trata de una asignatura cuantitativa o cualitativa
	$consulta = $db->consulta("SELECT id_tipo_asignatura FROM `sw_asignatura` WHERE id_asignatura = $id_asignatura");
	$registros = $db->fetch_assoc($consulta);
	$id_tipo_asignatura = $registros["id_tipo_asignatura"];
	// Aqui imprimo las cabeceras de cada rubrica de evaluacion
	$consulta = $db->consulta("SELECT ru_abreviatura FROM sw_rubrica_evaluacion r, sw_asignatura a WHERE r.id_tipo_asignatura = a.id_tipo_asignatura AND a.id_asignatura = $id_asignatura AND id_aporte_evaluacion = $id_aporte_evaluacion");
	...
	$consulta = $db->consulta("SELECT e.id_estudiante, e.es_apellidos, e.es_nombres FROM sw_estudiante_periodo_lectivo ep, sw_estudiante e WHERE ep.id_estudiante = e.id_estudiante AND ep.id_paralelo = $id_paralelo AND es_retirado = 'N' AND activo = 1 ORDER BY es_apellidos, es_nombres ASC");
	$num_total_registros = $db->num_rows($consulta);
	if($num_total_registros>0)
	{
		...
		while($paralelo = $db->fetch_assoc($consulta))
		{
			...
			if ($id_tipo_asignatura == 1) {
				$rubrica_evaluacion = $db->consulta("SELECT id_rubrica_evaluacion FROM sw_rubrica_evaluacion r, sw_asignatura a WHERE r.id_tipo_asignatura = a.id_tipo_asignatura AND a.id_asignatura = $id_asignatura AND id_aporte_evaluacion = $id_aporte_evaluacion");
				...
				if($num_total_registros>0)
				{
					...
					while($rubricas = $db->fetch_assoc($rubrica_evaluacion))
					{
						...
						$qry = $db->consulta("SELECT re_calificacion 
						...
						$pdf->Cell(14,6,($calificacion==0)?" ":sprintf('%0.2f', $calificacion),1,0,'C');
					}
					$promedio = $suma_rubricas / $contador_rubricas;
					$suma_promedios += $promedio;
					$pdf->Cell(14,6,($promedio==0)?" ":sprintf('%0.2f', substr($promedio, 0, strpos($promedio, '.') + 3)),1,0,'C');
					if($promedio > 0) $calificaciones_validas++;
				}	
			} else {
				$rubrica_evaluacion = $db->consulta("SELECT rc_calificacion 
														...
														AND id_asignatura = $id_asignatura");
				$num_total_registros = $db->num_rows($rubrica_evaluacion);
				$rubrica_estudiante = $db->fetch_assoc($rubrica_evaluacion);
				if($num_total_registros>0) {
					$calificacion = $rubrica_estudiante["rc_calificacion"];
				} else {
					$calificacion = " ";
				}
				$pdf->Cell(14,6,$calificacion,1,0,'C');
			}
			$pdf->Ln();
		}
	}
	if($id_tipo_asignatura == 1) {
		...
		$pdf->Cell(14,6,sprintf('%0.2f', $suma_promedios / $calificaciones_validas),1,0,'C');
		$pdf->Ln();
	}
	$pdf->Ln(2);
	$pdf->Cell(10,6,"Prof.: ",0,0,'L');
	$pdf->Cell(90,6,$nombreUsuario,0,0,'L');
	$pdf->Cell(30,6,"f.)____________________",0,0,'L');
	$pdf->Output();
?>
