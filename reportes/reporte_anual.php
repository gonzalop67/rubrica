<?php
	...
	class PDF extends FPDF
	{
		...
		function Footer()
		{
			...
			//N�mero de p�gina
			$this->Cell(0,10,'P�gina '.$this->PageNo().' de {nb} - Archivo: '.$filename,0,0,'C');
		}
	}
	...
	$db = new MySQL();
	$consulta = $db->consulta("SELECT e.id_estudiante, es_apellidos, es_nombres, es_genero, es_retirado FROM sw_estudiante e, sw_estudiante_periodo_lectivo p WHERE e.id_estudiante = p.id_estudiante AND p.id_paralelo = $id_paralelo AND activo = 1 ORDER BY es_apellidos, es_nombres ASC");
		...
		if($total_asignaturas>0)
		{
			...
			while ($asignatura = $db->fetch_assoc($asignaturas))
			{
				...
				if ($retirado == "S") {
					...
					$promedio_periodos = ($contador_sin_examen > 0) ? 'S/E' : number_format($promedio_periodos,2);			
