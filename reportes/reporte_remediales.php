<?php
	...
	class PDF extends FPDF
	{
		...
		function Header()
		{
			...
			$title2="REPORTE DE REMEDIALES PERIODO LECTIVO ".$this->nombrePeriodoLectivo;
	...
	if($num_total_registros>0)
	{
		...
		while($paralelo = $db->fetch_assoc($consulta))
		{
			...
			$query = $db->consulta("SELECT aprueba_todos_supletorios($id_periodo_lectivo, $id_estudiante, $id_paralelo) AS aprueba");
			...
			if(!$aprueba) {
				...
				$id_asignaturas = $db->consulta("SELECT a.id_asignatura 
												   FROM sw_asignatura a, 
												   		sw_asignatura_curso ac, 
														sw_paralelo p 
												  WHERE a.id_asignatura = ac.id_asignatura 
				...
				if($total_asignaturas>0)
				{
					...
					while ($asignatura = $db->fetch_assoc($id_asignaturas))
					{
						...
						// Obtengo el promedio anual del estudiante en esta asignatura
						...
						if($promedio_periodos >= 7 || 7 - $promedio_periodos < 0.01) {
							$pdf->Cell(13,5," ",1,0,'C');
							...
							if($calificacion < 7) {
								// Aqu� obtengo la calificaci�n del examen remedial
