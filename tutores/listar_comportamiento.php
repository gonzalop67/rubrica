...
function listarComportamientosPorTutores()
{
	...
	if ($num_total_registros > 0) {
		...
		while ($paralelo = $db->fetch_assoc($consulta)) {
			...
			$queryString = "SELECT co_calificacion, "
			...
			$queryString = $db->consulta("SELECT ac.ap_estado
			...
			$aporte = $db->fetch_assoc($queryString);
			...
			$qry = "SELECT ec_relacion FROM sw_escala_comportamiento WHERE id_escala_comportamiento = $id_escala_comportamiento";
