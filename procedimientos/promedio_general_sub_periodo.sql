DELIMITER $$
CREATE FUNCTION `promedio_general_sub_periodo` (`IdPeriodoEvaluacion` INT, `IdEstudiante` INT, `IdParalelo` INT) RETURNS FLOAT NO SQL
BEGIN
	...
	Lazo: LOOP
		...
		IF done THEN
			...
			LEAVE Lazo;
		END IF;
		SET suma = suma + (SELECT calcular_promedio_sub_periodo(IdPeriodoEvaluacion,IdEstudiante,IdParalelo,IdAsignatura));
